...
def transform(notice_id: str, soup: object) -> str:
    notice_json = {
        'is_french': True,
...
def create_task_parse(notice_id: str) -> str:
    notice_url = f'https://www.sudoc.fr/{notice_id}.xml'
    notice = requests.get(url=notice_url).text
    notice_xml = BeautifulSoup(notice, 'lxml')
    notice_json = transform(notice_id, notice_xml)
    set_objects([notice_xml], 'sudoc', 'notices')
    set_objects([notice_json], 'sudoc', 'parsed')
    return notice_json
